...
class manager:
    ...
    def buy_products(self, gid, uid, item, val) -> str:
        ...
        return f"成功花费{manager._format_cost(origin)}购买了{item}x{manager._format_num(val)}，剩余{self.backend[gid, uid]}金币"
    ...
    def sell_products(self, gid, uid, item, val) -> str:
        ...
        return f"成功卖出了{item}x{manager._format_num(val)}，获得了{manager._format_negcost(origin)}，现有{self.backend[gid, uid]}金币"
...
#余额查询
    def check_money(self, gid, uid):
        money = self.backend[gid, uid]
        return f"还有{money}金币。"

#每日签到
    def daily_check(self, gid, uid) -> str:
        if check_lmt.check(int(gid) * int(uid)):
            check_lmt.increase(int(gid) * int(uid))
            rand = random.randint((10), 50)
            if random.random() < 0.016:
                rand *= 100
                self.backend[gid, uid] += rand
                return f'签到完成，运气爆棚，共获得{rand}金币。'
            else:
                self.backend[gid, uid] += rand
                return f'签到完成，获得{rand}金币。'
        else:
            return f'今天已经签到过了，不要贪心哦。'
