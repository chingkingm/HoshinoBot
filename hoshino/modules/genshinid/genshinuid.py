    ...
    try:
        req = requests.get(
            ...
            headers = {
                ...
                'Cookie': 'UM_distinctid=1775b5bbaf65eb-0e2856a9cc28b5-13e3563-1fa400-1775b5bbaf7874; CNZZDATA1275023096=1548441067-1612144344-%7C1612144344; mi18nLang=zh-cn; _MHYUUID=e99d68fb-0f54-4eb3-b31e-e6b72dc7a868; login_uid=264475285; login_ticket=S5EFYZUAFENivBtJgRNhjS26lr2QfoFwrdyRqHFf; account_id=264475285; cookie_token=wqvdlEdFWtlmpSoznLxpK2xB7ahHVON0Y92CwNIM; ltoken=kFamfaSIZEm8wywH8gEyMX6BXq98bjJ6oOWBEPNu; ltuid=264475285',#自己获取
...
async def genshin(bot, ev: CQEvent):
    ...
    if (len(uid) == 9):
        if (uid[0] == "1"):
            ...
            sv.logger.info('原神查询uid中')
            UidInfo = JsonAnalysis(GetInfo(uid ,"cn_qd01"),uid ,"cn_qd01")
            sv.logger.info('原神uid查询成功')
            tes_list = []
            msg_text = f'UID{uid} (官服)的信息为：\r\n{UidInfo}'
            n = ImgText(msg_text)
...
async def genshin(bot: HoshinoBot, ev: CQEvent):
    ...
    if (len(uid) == 9):
        if (uid[0] == "1"):
            ...
            sv.logger.info('原神深渊查询中')
            SpiralAbysInfo = JsonSpiralAbys(GetSpiralAbys(uid ,"cn_qd01"), Schedule_type)
            sv.logger.info('原神深渊查询成功')
            tes_list = []
            msg_text = f'UID{uid} (官服)的深渊信息为：\r\n{SpiralAbysInfo}'
            n = ImgText(msg_text)
