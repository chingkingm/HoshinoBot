...
sv = Service('nga-spider', bundle='崩坏3订阅', help_='''
启用深渊战场爬虫 | 启用爬虫
禁用深渊战场爬虫 | 关闭爬虫服务
'''.strip())
...
@sv.on_prefix(('启用深渊战场爬虫', '启动深渊战场爬虫', '开启深渊战场爬虫'))
async def turn_on_spider(bot, ev: CQEvent):
    ...
    if save_file(config, 'spider_config.json'):
        ...
        await bot.send(ev, '启用深渊战场爬虫失败，请重试')


@sv.on_fullmatch(('关闭深渊战场爬虫', '禁用深渊战场爬虫'))
...
async def nga_spider():
    ...
    for section in broadcast_groups.keys():
        if broadcast_groups[section]:
            ...
            await spider_work(NGASpider, bot, section, broadcast_groups[section], sv, '深渊战场爬虫')
